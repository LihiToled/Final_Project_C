...
bool preprocess_file(char *filename){
    ...
    while(fgets(current_line, MAX_LINE_LENGTH+2,origin_file_des)){
        ...
        split_line = split_string(current_line + index, " ", &number_of_words_in_line); /* Split the line from where whitespace ends */
        
        /* If defined macro, print it */
        if(get_from_m_table(macros_table, split_line[0]) != NULL){
            fputs(get_from_m_table(macros_table, split_line[0]), am_file_des);
        }
        /* If there's a macro definition */
        else if (strncmp(split_line[0], "mcro",3) == 0){
            reading_macro = TRUE;
            macro_name = (char *)malloc(sizeof(char) * MAX_MACRO_NAME_SIZE);
            strcpy(macro_name, split_line[1]);
            add_m_entry(macros_table, macro_name, "");
        }
        /* If there's an end of macro definition */
        else if (strncmp(split_line[0], "endmcro",6) == 0){
            reading_macro = FALSE;
        }
        ...
        /* Free the memory of split line before iterating to the next line */
        free_split_string(split_line, number_of_words_in_line);
