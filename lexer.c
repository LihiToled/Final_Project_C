    ...
    if(line[index] == '.'){
        ...
        if(lt.asm_inst_asm_dir.asm_inst.inst_name == lex_inst_stop || lt.asm_inst_asm_dir.asm_inst.inst_name == lex_inst_rts){
            ...
            if(line[index] != '\0' && line[index] != '\n' && line[index] != EOF){
                lt.lex_union_option = lex_union_lex_error;
                strcpy(lt.lex_error, "Detected extra characters, Group C instructions do not have operands after command name.");
                return lt;
            }
            return lt;
        }
...
static void get_string(const char *line, int *index, struct lex_tree *lt){
    ...
    if(line[*index] != '\0' && line[*index] != '\n' && line[*index] != EOF){
        lt->lex_union_option = lex_union_lex_error;
        strcpy(lt->lex_error, "Detected charachters after the end of the string.");
        return;
    }
}
...
static void get_data(const char *line, int *index, struct lex_tree *lt){
    ...
    do{
        ...
        if(line[*index] == '\0' || line[*index] == '\n' || line[*index] == EOF){
            ...
            if(line[*index] != ','){
                lt->lex_union_option = lex_union_lex_error;
                strcpy(lt->lex_error, "Invalid character, expected a comma.");
                return;
            }
            (*index)++; /* Skip the ',' */
            SKIP_WHITE(line, (*index))
...
static void get_ent_extern_label(const char *line, int *index, struct lex_tree *lt){
    ...
    SKIP_WHITE(line, (*index))
    if(lt->label[0] != '\0'){
        lt->lex_union_option = lex_union_lex_warn;
        strcpy(lt->lex_error, "Entry/Extern instruction should not be under label definition.");
    }
    if(line[*index] == '\0' || line[*index] == '\n' || line[*index] == EOF){
        lt->lex_union_option = lex_union_lex_error;
...
static void get_code_instruction(const char *line, int *index, struct lex_tree *lt){
    ...
    lt->lex_union_option = lex_union_asm_inst;
    {
        ...
        lt->lex_union_option = lex_union_lex_error;
        strcpy(lt->lex_error, "Command does not exist.");
    }
...
static void get_set_a_operands(const char *line, int *index, struct lex_tree *lt){
    ...
    if(lt->lex_union_option == lex_union_lex_error){
        strcpy(lt->lex_error, "First operand of command is illegal.");
        free(operand);
        return;
    }
    else{
        if(lt->asm_inst_asm_dir.asm_inst.asm_inst_sets.asm_inst_setA.oat[0] == lex_op_addr_type_immed){
            ...
            lt->asm_inst_asm_dir.asm_inst.asm_inst_sets.asm_inst_setA.oatu[0].reg = operand[2];
        }
    }
    free(operand);
    (*index) += len;
    ...
    if(lt->lex_union_option == lex_union_lex_error){
        strcpy(lt->lex_error, "Second operand of command is illegal.");
        free(operand);
        return;
    }
    else{
        if(lt->asm_inst_asm_dir.asm_inst.asm_inst_sets.asm_inst_setA.oat[1] == lex_op_addr_type_immed){
            ...
            lt->asm_inst_asm_dir.asm_inst.asm_inst_sets.asm_inst_setA.oatu[1].reg = operand[2] ;
        }
    }
    free(operand);
    (*index) += len;
...
static void get_set_b_operands(const char *line, int *index, struct lex_tree *lt){
    ...
    if(line[*index] != '\0' && line[*index] != '\n' && line[*index] != EOF){
        lt->lex_union_option = lex_union_lex_error;
        strcpy(lt->lex_error, "Detected trailing characters after the operand. Group B allows only one operand.");
        free(operand);
        return;
    }
    if(strchr(operand, '(') == NULL){
        ...
        if(lt->lex_union_option == lex_union_lex_error){
            ...
            if(lt->asm_inst_asm_dir.asm_inst.inst_name == lex_inst_jsr || lt->asm_inst_asm_dir.asm_inst.inst_name == lex_inst_bne ||
               lt->asm_inst_asm_dir.asm_inst.inst_name == lex_inst_jmp){
                if(!is_valid_label(operand)){
                    lt->lex_union_option = lex_union_lex_error;
                    strcpy(lt->lex_error, "Invalid parameter. ");
                    free(operand);
                    return;
                }
            }
            if(lt->asm_inst_asm_dir.asm_inst.asm_inst_sets.asm_inst_setB.oat == lex_op_addr_type_immed){
                ...
                lt->asm_inst_asm_dir.asm_inst.asm_inst_sets.asm_inst_setB.setB_operands.oatu.reg = operand[2];
            }
            free(operand);
            return;
        }
...
static enum operand_addr_type analyze_operand(char *operand, struct  lex_tree *lt){
    ...
    if (is_all_digits(operand)||(*operand=='-')) {
        /* Immediate number */
        value_str = &operand[0];
        ...
        for (i = 0; i < strlen(value_str); i++) {
            if (!isdigit(value_str[i]) && value_str[i] != '-') {
                ...
                lt->lex_union_option = lex_union_lex_error;
                strcpy(lt->lex_error, "Non-digit character was detected in operand.");
                return lex_op_addr_type_error;
            }
        }
        ...
        if (*endptr != '\0') {
            ...
            lt->lex_union_option = lex_union_lex_error;
            strcpy(lt->lex_error, "Invalid integer.");
            return lex_op_addr_type_error;
        }
